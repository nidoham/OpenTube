...
import com.nidoham.opentube.dialog.ScaleModeDialog;
...
public class PlayerControlsManager {
    ...
    public PlayerControlsManager(Context context, PlayerControlsCallback callback, VideoScaleManager vsm) {
        this.context = context;
        this.callback = callback;
        this.videoScaleManager = vsm;
        this.sharedPreferences = context.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE);

        // Load high quality support from SharedPreferences
        this.supportsHighQuality = sharedPreferences.getBoolean(PREF_SUPPORTS_HIGH_QUALITY, DEFAULT_HIGH_QUALITY_SUPPORT);
    ...
    public void showSettingsDialog() {
        ...
        String scaleModeName = videoScaleManager != null ? videoScaleManager.getCurrentScaleModeName() : "Unknown";
    ...
    public void showScaleModeDialog() {
        if (videoScaleManager == null) return;

        int currentMode = videoScaleManager.getCurrentScaleMode();
        boolean isLandscape = videoScaleManager.isLandscapeMode();

        ScaleModeDialog.showEnhanced(
                context,
                currentMode,
                isLandscape,
                selectedMode -> {
                    videoScaleManager.setScaleMode(selectedMode);

                    // Show feedback
                    String modeName = VideoScaleManager.getScaleModeName(selectedMode);
                    Toast.makeText(context, "Scale: " + modeName, Toast.LENGTH_SHORT).show();
                });
    }
    ...
    public int getCurrentScaleMode() {
        if (videoScaleManager != null) {
            return videoScaleManager.getCurrentScaleMode();
        }
        return VideoScaleManager.SCALE_DEFAULT;
    }
    ...
    /**
     * Gets the VideoScaleManager instance (instance method, not static)
     */
    public VideoScaleManager getVideoScaleManager() {
        return videoScaleManager;
    }

    /**
     * Sets the VideoScaleManager instance
     */
    public void setVideoScaleManager(VideoScaleManager videoScaleManager) {
        this.videoScaleManager = videoScaleManager;
    }
}